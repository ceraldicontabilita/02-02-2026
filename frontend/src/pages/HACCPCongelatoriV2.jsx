...
export default function HACCPCongelatoriV2() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const promises = [];
      for (let i = 1; i <= 12; i++) {
        promises.push(api.get(`/api/haccp-v2/temperature-negative/scheda/${anno}/${i}`));
      }
  ...
  const popolaCongelatori = async () => {
    try {
      await api.post(`/api/haccp-v2/temperature-negative/popola/${anno}`);
  ...
  const stampaScheda = () => {
    ...
    win.document.write(`<!DOCTYPE html><html><head><title>Temperature Congelatori ${MESI_IT[mese-1]} ${anno}</title>
      <style>body{font-family:Arial;font-size:10pt;margin:15mm}table{border-collapse:collapse;width:100%}th{background:#e3f2fd;padding:4px;border:1px solid #ccc}</style>
      </head><body>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12, marginBottom: 16 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 22, display: 'flex', alignItems: 'center', gap: 8 }}>
            ❄️ Temperature Congelatori (1-12)
          </h1>
