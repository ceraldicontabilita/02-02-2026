          ...
          {activeTab === 'f24' && (
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '24px' }}>
                <StatCard 
                  title="F24 Totali" 
                  value={f24StatoRiconciliazione?.f24?.totali || 0}
                  icon={FileText}
                  color="#3b82f6"
                />
                <StatCard 
                  title="F24 Pagati" 
                  value={f24StatoRiconciliazione?.f24?.pagati || 0}
                  icon={CheckCircle2}
                  color="#10b981"
                />
                <StatCard 
                  title="F24 Da Pagare" 
                  value={f24StatoRiconciliazione?.f24?.da_pagare || 0}
                  icon={Clock}
                  color="#f59e0b"
                />
                <StatCard 
                  title="% Riconciliato" 
                  value={`${f24StatoRiconciliazione?.percentuale_riconciliazione || 0}%`}
                  icon={Link2}
                  color="#8b5cf6"
                />
              </div>
              ...
              <div style={{ 
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                  ...
                  <button
                    onClick={riconciliaF24}
                    disabled={riconciliando}
                    style={{
                      padding: '8px 16px',
                      background: riconciliando ? '#e2e8f0' : 'linear-gradient(135deg, #3b82f6 0%, #2563eb 100%)',
                      color: riconciliando ? '#9ca3af' : 'white',
                      border: 'none',
                      borderRadius: '8px',
                    ...
                    data-testid="btn-riconcilia-f24"
                  >
                    <Link2 size={16} style={riconciliando ? { animation: 'spin 1s linear infinite' } : {}} />
                    {riconciliando ? 'Riconciliazione...' : 'Riconcilia F24 ↔ Banca'}
                  </button>
                </div>
                
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px' }}>
                  <div style={{ padding: '16px', background: '#f0fdf4', borderRadius: '8px', textAlign: 'center' }}>
                    <div style={{ fontSize: '12px', color: '#16a34a', fontWeight: 500, marginBottom: '4px' }}>F24 Pagati</div>
                    <div style={{ fontSize: '20px', fontWeight: 700, color: '#15803d' }}>
                      {formatEuro(f24StatoRiconciliazione?.importi?.totale_f24_pagati || 0)}
                    </div>
                  </div>
                  <div style={{ padding: '16px', background: '#fef3c7', borderRadius: '8px', textAlign: 'center' }}>
                    <div style={{ fontSize: '12px', color: '#d97706', fontWeight: 500, marginBottom: '4px' }}>F24 Da Pagare</div>
                    <div style={{ fontSize: '20px', fontWeight: 700, color: '#b45309' }}>
                      {formatEuro(f24StatoRiconciliazione?.importi?.totale_f24_da_pagare || 0)}
                    </div>
                  </div>
                  <div style={{ padding: '16px', background: '#dbeafe', borderRadius: '8px', textAlign: 'center' }}>
                    <div style={{ fontSize: '12px', color: '#2563eb', fontWeight: 500, marginBottom: '4px' }}>Movimenti Banca</div>
                    <div style={{ fontSize: '20px', fontWeight: 700, color: '#1d4ed8' }}>
                      {formatEuro(f24StatoRiconciliazione?.importi?.totale_movimenti_banca || 0)}
                    </div>
                  </div>
                </div>
                ...
                {risultatoRiconciliazione && (
                  <div style={{ 
                    marginTop: '20px', 
                    padding: '16px', 
                    background: risultatoRiconciliazione.success ? '#f0fdf4' : '#fef2f2', 
                    borderRadius: '8px',
                    border: `1px solid ${risultatoRiconciliazione.success ? '#86efac' : '#fecaca'}`
                  }}>
                    <h4 style={{ margin: '0 0 12px 0', color: risultatoRiconciliazione.success ? '#16a34a' : '#dc2626', fontWeight: 600 }}>
                      {risultatoRiconciliazione.message}
                    </h4>
                    {risultatoRiconciliazione.stats && (
                      <div style={{ fontSize: '13px', color: '#374151' }}>
                        <p>✅ Riconciliati: {risultatoRiconciliazione.stats.riconciliati || 0}</p>
                        <p>⏳ Non pagati: {risultatoRiconciliazione.stats.non_pagati || 0}</p>
                        <p>❓ Movimenti non associati: {risultatoRiconciliazione.stats.movimenti_non_associati || 0}</p>
                      </div>
                    )}
                  </div>
                )}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
                ...
                <div style={{ 
                  ...
                  <h3 style={{ fontSize: '16px', fontWeight: 600, color: '#1f2937', marginBottom: '16px' }}>
                    F24 Commercialista ({f24List.length})
                  </h3>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    {f24List.slice(0, 20).map((f24, i) => {
                      const dg = f24.dati_generali || {};
                      const tot = f24.totali || {};
                      const stato = f24.status || f24.stato_pagamento || 'non_classificato';
                      return (
                        <div 
                          key={f24.id || i}
                          style={{
                        ...
                        >
                          <div>
                            <div style={{ fontWeight: 600, color: '#1f2937', fontSize: '14px' }}>
                              {dg.data_versamento || 'N/D'}
                            </div>
                            <div style={{ fontSize: '12px', color: '#64748b' }}>
                              {f24.file_name?.substring(0, 30) || 'File N/D'}
                            </div>
                          </div>
                          <div style={{ textAlign: 'right' }}>
                            <div style={{ fontWeight: 700, color: '#1f2937' }}>
                              {formatEuro(tot.saldo_netto || tot.saldo_finale || 0)}
                            </div>
                            <span style={{
                              padding: '2px 8px',
                              borderRadius: '4px',
                              fontSize: '11px',
                              fontWeight: 600,
                              background: stato === 'pagato' ? '#dcfce7' : stato === 'da_pagare' ? '#fef3c7' : '#f1f5f9',
                              color: stato === 'pagato' ? '#16a34a' : stato === 'da_pagare' ? '#d97706' : '#64748b'
                            }}>
                ...
                <div style={{ 
                  ...
                  <h3 style={{ fontSize: '16px', fontWeight: 600, color: '#1f2937', marginBottom: '16px' }}>
                    Quietanze F24 ({quietanze.length})
                  </h3>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    {quietanze.slice(0, 20).map((q, i) => {
                      ...
                      return (
                        ...
                        >
                          <div>
                            <div style={{ fontWeight: 600, color: '#1f2937', fontSize: '14px' }}>
                              {dg.data_pagamento || 'N/D'}
                            </div>
                            <div style={{ fontSize: '12px', color: '#64748b' }}>
                              Prot: {dg.protocollo_telematico?.substring(0, 20) || 'N/D'}...
                            </div>
                          </div>
                          <div style={{ textAlign: 'right' }}>
