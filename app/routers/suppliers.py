...
async def list_suppliers(
    ...
    for item in invoice_suppliers:
        ...
        # Prende il nome - priorità a supplier_name (campo XML), poi fornitore.denominazione
        nome = (
        ...
        suppliers_map[piva] = {
            "id": f"inv_{piva}",
            "partita_iva": piva,
            "codice_fiscale": fornitore.get("codice_fiscale", piva),
...
@router.post("/import-excel")
async def import_fornitori_excel(file: UploadFile = File(...)) -> Dict[str, Any]:
    """
    Importa fornitori da file Excel.
    Per fornitori senza Denominazione, usa Cognome + Nome.
    """
    import pandas as pd
    import io
    ...
    risultato = {"aggiornati": 0, "nuovi": 0, "skipped": 0, "errori": []}
    
    for idx, row in df.iterrows():
        try:
            piva = str(row.get('Partita Iva', '')).strip()
            if not piva or piva == 'nan':
                continue
            
            # Costruisci denominazione
            denominazione = str(row.get('Denominazione', '')).strip()
            if not denominazione or denominazione == 'nan':
                cognome = str(row.get('Cognome', '')).strip()
                nome = str(row.get('Nome', '')).strip()
                if cognome and cognome != 'nan':
                    denominazione = f"{cognome} {nome}".strip() if nome and nome != 'nan' else cognome
            ...
            dati = {
                ...
                "codice_fiscale": str(row.get('Codice Fiscale', '')).strip() if pd.notna(row.get('Codice Fiscale')) else piva,
                "email": str(row.get('Email', '')).strip() if pd.notna(row.get('Email')) else "",
                "pec": str(row.get('PEC', '')).strip() if pd.notna(row.get('PEC')) else "",
                "telefono": str(row.get('Telefono', '')).strip() if pd.notna(row.get('Telefono')) else "",
                "indirizzo": str(row.get('Indirizzo', '')).strip() if pd.notna(row.get('Indirizzo')) else "",
                "cap": str(row.get('CAP', '')).strip() if pd.notna(row.get('CAP')) else "",
                "comune": str(row.get('Comune', '')).strip() if pd.notna(row.get('Comune')) else "",
                "provincia": str(row.get('Provincia', '')).strip() if pd.notna(row.get('Provincia')) else "",
                "nazione": str(row.get('Nazione', 'IT')).strip() if pd.notna(row.get('Nazione')) else "IT",
                "updated_at": datetime.utcnow().isoformat()
            ...
            if esistente:
                ...
                dati["metodo_pagamento"] = "bonifico"
                dati["attivo"] = True
                dati["created_at"] = datetime.utcnow().isoformat()
                await db[Collections.SUPPLIERS].insert_one(dati.copy())
                risultato["nuovi"] += 1
...
async def update_supplier_nome(
    supplier_id: str,
    data: Dict[str, Any] = Body(...)
) -> Dict[str, Any]:
    """
