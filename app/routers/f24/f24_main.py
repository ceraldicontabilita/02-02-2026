...
# ============== SCADENZE PROSSIME (per Dashboard) ==============
@router.get(
    "/scadenze-prossime",
    summary="Get upcoming F24 deadlines for dashboard"
)
async def get_scadenze_prossime(
    giorni: int = 60,
    limit: int = 5
) -> Dict[str, Any]:
    """
    Get upcoming F24 deadlines for the dashboard widget.
        ...
        try:
            ...
            if giorni_mancanti <= giorni:
                ...
                scadenze.append({
                    ...
                    "codice_tributo": codici[0].get("codice", "") if codici and isinstance(codici[0], dict) else (codici[0] if codici else ""),
                    "tributo": f24.get("tributo", ""),
                    "importo": importo,
            ...
            logger.debug(f"Error parsing F24 date for dashboard: {e}")
            continue
