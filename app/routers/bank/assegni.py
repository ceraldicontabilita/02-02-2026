...
async def verifica_associazioni_assegni() -> Dict[str, Any]:
    ...
    for f in fatture_cursor:
        key = f"{f.get('invoice_number', '')}_{f.get('supplier_vat', '')}"
        fatture_by_number[key] = f
    ...
    for assegno in assegni:
        ...
        if not fattura:
            ...
            fatture_simili = [
                ...
                if abs(float(f.get("total_amount", 0)) - importo_assegno) < 5
            ]
            if fatture_simili:
        ...
        if beneficiario and fornitore:
            ...
            if similarity < 60:
                problema["problemi"].append(f"Beneficiario '{beneficiario}' diverso da fornitore '{fornitore}' (match: {similarity}%)")
        ...
        if data_assegno and data_fattura:
            try:
                ...
                if giorni_differenza > 180:
                    problema["problemi"].append(f"Data assegno e fattura differiscono di {giorni_differenza} giorni")
        ...
        if ha_problemi:
            ...
            # Cerca per beneficiario/fornitore simile
            for f in fatture_cursor:
...
async def correggi_associazione_assegno(
    assegno_id: str,
    nuova_fattura_id: Optional[str] = Body(None, description="ID della nuova fattura da associare (null per rimuovere)"),
    aggiorna_beneficiario: bool = Body(False, description="Aggiorna beneficiario dal fornitore fattura")
) -> Dict[str, Any]:
    """
    Corregge l'associazione di un assegno con una fattura.
    
    - Se nuova_fattura_id è fornito, associa la nuova fattura
    - Se nuova_fattura_id è null, rimuove l'associazione
    - Se aggiorna_beneficiario è True, aggiorna anche il beneficiario
    """
    db = Database.get_db()
    ...
    if nuova_fattura_id:
        ...
        # Marca la vecchia fattura come non pagata (se esisteva)
        ...
        # Marca la nuova fattura come collegata all'assegno
        await db["invoices"].update_one(
            {"id": nuova_fattura_id},
            {"$set": {"assegno_id": assegno_id}}
        )
        
        message = f"Associazione corretta: {assegno.get('numero_assegno') or assegno.get('numero')} -> Fattura {update_data['numero_fattura']}"
    else:
        # Rimuovi l'associazione
        update_data["fattura_id"] = None
        update_data["numero_fattura"] = None
        
        # Marca la vecchia fattura come non pagata
        if vecchia_fattura_id:
            await db["invoices"].update_one(
                {"id": vecchia_fattura_id},
                {"$set": {"pagato": False, "status": "imported", "assegno_id": None}}
