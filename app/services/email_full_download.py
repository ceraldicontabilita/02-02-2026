...
async def smart_auto_associate(db: AsyncIOMotorDatabase) -> Dict[str, int]:
    ...
    for category, rules in association_rules.items():
        source_coll = CATEGORY_COLLECTIONS.get(category)
        if not source_coll:
            continue
        
        # Trova PDF non associati di questa categoria
        cursor = db[source_coll].find({"associato": False})
        
        async for pdf_doc in cursor:
            try:
                filename = pdf_doc.get("filename", "")
                
                # Cerca documento corrispondente
                target = await db[rules["target_collection"]].find_one({
                    "$or": [
                        {"file_name": filename},
                        {"filename": filename},
                        {"file_name": {"$regex": filename[:20], "$options": "i"}}
                    ],
                    "pdf_data": {"$in": [None, ""]}  # Solo se non ha già PDF
                })
                ...
                if target:
                    success = await associate_pdf_to_document(
                        ...
                        source_coll,
                        target["id"],
                        rules["target_collection"]
                    )
                    if success:
                        stats["associated"] += 1
                        logger.info(f"Auto-associato: {filename}")
                    else:
                        stats["errors"] += 1
                else:
                    stats["skipped"] += 1
                ...
                logger.error(f"Errore auto-associazione: {e}")
                stats["errors"] += 1
